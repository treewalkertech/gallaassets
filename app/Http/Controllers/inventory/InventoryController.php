...
class InventoryController extends Controller
{
    ...
    {
        // 1) Collect ids from route param / request
        ...
        } elseif (! empty($singleId)) {
            $ids = [$singleId];
        } else {
            return response()->json(['success' => false, 'message' => 'No id(s) provided.'], 422);
        }

        Log::info('Delete requested for RFID Tag ids: '.json_encode($ids));

        DB::beginTransaction();
        try {
            // 2) Fetch tags with product relation (for display / checks) and apply location restriction
            ...
            foreach ($tags as $tag) {
                try {
                    // OPTIONAL: skip tags that are written/programmed and should not be deleted
                    // if (isset($tag->is_write) && intval($tag->is_write) === 1) {
                    //     $skipped[] = $tag->id;
                    //     continue;
                    // }

                    // Delete related inventory_activity rows (if you want to remove history)
                    try {
                        \App\Models\inventory\InventoryActivity::where('inventory_id', $tag->id)->delete();
                    } catch (\Throwable $t) {
                        // log and continue — not fatal
                        Log::warning("Failed deleting inventory_activity for tag {$tag->id}: ".$t->getMessage());
                    }
                    ...
                    Log::error("Failed to delete tag id {$tag->id}: ".$t->getMessage());
                    $failed[] = ['id' => $tag->id, 'error' => $t->getMessage()];
                }
            }
            ...
            $messageParts = [];
            $messageParts[] = "{$deletedCount} item(s) deleted successfully.";
            if (! empty($skipped)) {
                $messageParts[] = (count($skipped).' item(s) skipped.');
            }
            if (! empty($failed)) {
                $messageParts[] = (count($failed).' item(s) failed to delete.');
            }
            $message = implode(' ', $messageParts);

            // User activity log (non-fatal)
            try {
                $user = Auth::user();
                $this->UserActivityLog($request, [
                    'module' => 'inventory',
                    'activity_type' => 'delete',
                    'message' => 'Bulk delete by: '.($user->fullname ?? $user->name ?? 'Unknown'),
                    'application' => 'web',
                    'data' => [
                        'requested_ids' => $ids,
                        ...
                        'skipped' => $skipped,
                        'failed' => $failed,
                        'tags' => $tags->toArray(),   // <-- correct & safe
                    ],
                ]);
            } catch (\Throwable $t) {
