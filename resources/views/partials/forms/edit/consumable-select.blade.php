    ...
    <div class="col-md-1 col-sm-1 text-left">
        @can('create', \App\Models\Consumable::class)
            @if ((!isset($hide_new)) || ($hide_new!='true'))
                <a href='{{ route('modal.show', 'consumable') }}' data-toggle="modal" data-target="#createModal" data-select='{{ (isset($select_id)) ? $select_id : 'assigned_consumable_select' }}' class="btn btn-sm btn-primary">{{ trans('button.new') }}</a>
            @endif
        @endcan
    </div>
